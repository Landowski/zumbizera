...
let savedName = localStorage.getItem("playerName") || "Player";
...
function updateRenderPositions(dt) {
  const seen = new Set();
  latestState.forEach((p) => {
    seen.add(p.id);
    let r = renderPositions.get(p.id);

    if (!r || r.room !== p.room) {
      renderPositions.set(p.id, { x: p.x, y: p.y, room: p.room });
      return;
    }

    const t = 1 - Math.exp(-POSITION_SMOOTH_RATE * dt);
    r.x += (p.x - r.x) * t;
    r.y += (p.y - r.y) * t;
  });

  [...renderPositions.keys()].forEach((id) => {
    if (!seen.has(id)) renderPositions.delete(id);
  });
}
...
function drawPlayerSprite(p, isHidden = false) {
  ...
  if (!isHidden) {
    ...
    const nameY = drawY - 6;
    const nameText = p.name ? p.name : "Player";
